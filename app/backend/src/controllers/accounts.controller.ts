...
export async function createAccount(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (result && result.success === false) throw Object.assign(new Error(result.error || 'Failed to create account'), { statusCode: 400 });
...
export async function addTransaction(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (result && result.success === false) throw Object.assign(new Error(result.error || 'Failed to add transaction'), { statusCode: 400 });
...
export async function listBalanceSheets(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const sheets = await db.getBalanceSheets(limit);
...
export async function generateBalanceSheet(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (result && result.success === false) throw Object.assign(new Error(result.error || 'Failed to generate balance sheet'), { statusCode: 400 });
    res.status(201).json({ success: true, data: result });
  } catch (err) { next(err); }
}
