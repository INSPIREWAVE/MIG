...
async function initDB(userDataPath) {
  ...
  const ALLOWED_TABLES = ['clients', 'loans', 'payments', 'penalties', 'audit_log', 'settings', 
    ...
    'balance_sheets', 'users', 'collateral', 'schema_versions', 'loan_installments'];
...
module.exports = {
  ...
  setSetting: (key, value) => { db.run(`INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)`, [key, value]); saveDB(); return { success: true }; },
  // Audit
  ...
  createLoanWithSchedule: (loanData) => {
    try {
      const { clientId, amount, interest, loanType = 'monthly', duration = 1, 
              loanDate, notes, disbursementDate, earlySettlementEnabled = 1 } = loanData;
      ...
      // Insert loan record with early settlement flag
